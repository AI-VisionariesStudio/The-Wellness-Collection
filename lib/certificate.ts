...
export async function issueCertificate(userId: string, courseId: string) {
  ...
  const certHtml = buildCertificateHTML({ user, course, serialNumber, issuedDate })
...
export function buildCertificateHTML({ user, course, serialNumber, issuedDate }: {
  user: { name: string; caseNumber?: string | null; courtCounty?: string | null; courtState?: string | null }
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .seal { width: 80px; height: 80px; border: 3px solid #D1C8BF; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 9px; color: #7a7065; text-align: center; letter-spacing: 1px; text-transform: uppercase; line-height: 1.4; }
</style>
</head>
<body>
<div class="cert">
  ...
  <div class="subtitle">Court-Approved Education</div>
  ...
  <div class="hours">${course.duration} Hours of Instruction &nbsp;|&nbsp; Issued ${issuedDate}</div>
  ${user.caseNumber ? `<div style="font-size:12px;color:#888;margin-bottom:20px;">Case No: ${user.caseNumber} &nbsp;|&nbsp; ${user.courtCounty ?? ''} County, ${user.courtState ?? ''}</div>` : ''}
  <div class="footer">
    <div class="sig-line">Authorized Signature</div>
    <div class="seal">Court<br>Approved<br>✓</div>
